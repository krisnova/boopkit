...
void
create_data_packet (struct sockaddr_in *src, struct sockaddr_in *dst,
...
void
create_syn_packet (struct sockaddr_in *src, struct sockaddr_in *dst,
                   char **out_packet, int *out_packet_len)
{
  char *datagram = calloc (DATAGRAM_LEN, sizeof (char));
  struct iphdr *iph = (struct iphdr *)datagram;
  ...
  int psize
      = sizeof (struct pseudo_header) + sizeof (struct tcphdr) + OPT_SIZE;
  // fill pseudo packet
  char *pseudogram = malloc (psize);
  memcpy (pseudogram, (char *)&psh, sizeof (struct pseudo_header));
  memcpy (pseudogram + sizeof (struct pseudo_header), tcph,
...
void
create_ack_packet (struct sockaddr_in *src, struct sockaddr_in *dst,
...
void
create_ack_rst_packet (struct sockaddr_in *src, struct sockaddr_in *dst,
                       int32_t seq, int32_t ack_seq, char **out_packet,
                       int *out_packet_len)
{
  char *datagram = calloc (DATAGRAM_LEN, sizeof (char));
...
void
create_rst_packet (struct sockaddr_in *src, struct sockaddr_in *dst,
                   char **out_packet, int *out_packet_len)
{
  char *datagram = calloc (DATAGRAM_LEN, sizeof (char));
  struct iphdr *iph = (struct iphdr *)datagram;
...
void
create_bad_syn_packet (struct sockaddr_in *src, struct sockaddr_in *dst,
                       char **out_packet, int *out_packet_len)
{
  char *datagram = calloc (DATAGRAM_LEN, sizeof (char));
  struct iphdr *iph = (struct iphdr *)datagram;
...
void
read_seq_and_ack (const char *packet, uint32_t *seq, uint32_t *ack)
{
...
int
receive_from (int sock, char *buffer, size_t buffer_length,
